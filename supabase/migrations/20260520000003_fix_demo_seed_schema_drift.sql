-- =============================================================================
-- Fix schema drift uncovered while seeding demo money-intelligence data.
--
-- The May 12-13 migrations were lost locally (see schema-snapshot-2026-05-20.sql).
-- Remote shows two divergences from the local migration files:
--
--   1. invoice_items.quantity was renamed to invoice_items.quantity_invoiced.
--      Frontend code already uses quantity_invoiced (see src/lib/usage-analytics.ts,
--      src/domain/invoices/buildComparisonRows.ts, types codegen).
--
--   2. waste_log lost its costing columns (unit_cost, total_cost, catalog_item_id)
--      even though 20260307000003_add_waste_costing.sql claims to add them.
--      src/domain/dashboard/loadWasteMetrics.ts still SELECTs them — that loader
--      is silently broken on remote until these columns return.
--
-- Then re-applies create_restaurant_with_owner with `quantity_invoiced` so the
-- demo seed can run. Body of the function is otherwise identical to
-- 20260520000002.
-- =============================================================================

-- ── 1. invoice_items: rename quantity → quantity_invoiced if old name exists ──
DO $$
BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'invoice_items' AND column_name = 'quantity_invoiced'
  ) THEN
    ALTER TABLE public.invoice_items RENAME COLUMN quantity TO quantity_invoiced;
  END IF;
END $$;

-- ── 2. waste_log: restore costing columns the loader expects ──────────────────
...
-- ── 3. Re-apply create_restaurant_with_owner with quantity_invoiced ───────────
...
BEGIN
  ...
  IF p_is_demo THEN
    ...
    -- Invoice line items (column is quantity_invoiced on remote, not quantity)
