-- S0-9: require a manager/owner to confirm invoice receipt.
--
-- Problem: confirm_invoice_receipt (and its sibling confirm_invoice_receipt_legacy)
-- authorized on is_member_of(p_restaurant_id) ONLY, so any member — incl. STAFF —
-- could perform the product's most destructive write: overwrite canonical catalog
-- costs (inventory_catalog_items.default_unit_cost), create receiving stock_movements,
-- mark invoices confirmed, and emit PRICE_INCREASE/DECREASE/UNIT_MISMATCH
-- notifications that feed money KPIs — despite the UI's "confirmed by a manager"
-- promise and the Manager+ StaffRestricted route (G9).
--
-- Fix: replace the membership preamble with the S0-INFRA helper
-- can_confirm_receipt(auth.uid(), p_restaurant_id) (Manager+, OWNER-inclusive;
-- shipped 20260623000005). can_confirm_receipt implies membership, so this both
-- authenticates and authorizes. Gated on BOTH functions (the legacy one is granted
-- to authenticated and directly callable). Fail closed BEFORE any write. The rest
-- of each body is reproduced verbatim from the live definitions (20260524000001 /
-- 20260507000001) — only the preamble changes. No client/UI/signature change.

-- ── confirm_invoice_receipt (live: 20260524000001) ─────────────────────────────
...
-- ── confirm_invoice_receipt_legacy (live: 20260507000001) ──────────────────────
...
DECLARE
  ...
  v_results           jsonb := '[]'::jsonb;
BEGIN
  -- S0-9: only a manager/owner may confirm receipt (same gate as the primary fn;
  -- this fn is granted to authenticated and directly callable). Fail closed.
  IF NOT public.can_confirm_receipt(auth.uid(), p_restaurant_id) THEN
    RAISE EXCEPTION 'Access denied: a manager or owner role is required to confirm receipt'
      USING ERRCODE = 'insufficient_privilege';
  END IF;
